...
The exception is worth naming, because it shows the pattern is a tendency rather than a law. Picovoice sells Falcon, benchmarks it against pyannote, and then [declines to pick a winner](#ref-picovoice-diarization): "Our goal was not to crown a single 'winner', but to understand tradeoffs between research accuracy and production efficiency." Its own numbers have [pyannote ahead of Falcon on DER, 9.0% against 10.3%](#ref-picovoice-diarization), with Falcon ahead on the Jaccard error rate. A vendor publishing a benchmark its own product loses is the rarest thing in this survey.
