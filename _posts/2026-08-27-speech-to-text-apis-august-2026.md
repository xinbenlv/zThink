...
These are not five independent measurements converging on a truth. Four of the five are marketing assets, and each one puts its publisher on top.

The exception is worth naming, because it shows the pattern is a tendency rather than a law. Picovoice sells Falcon, benchmarks it against pyannote, and then [declines to pick a winner](#ref-picovoice-diarization): "Our goal was not to crown a single 'winner', but to understand tradeoffs between research accuracy and production efficiency." Its own numbers have pyannote ahead of Falcon on DER, 9.0% against 10.3%, with Falcon ahead on the Jaccard error rate. A vendor publishing a benchmark its own product loses is the rarest thing in this survey.
...
- **Gemini's figure is Google's own estimate.** Billing is per token, and the page converts it to "an effective blended rate of ~$0.005 per min for Transcribe" assuming 25 audio tokens per second in and 175 text tokens per minute out. Talkative audio produces more output tokens and costs more, so treat $0.30/hr as a floor rather than a quote.
