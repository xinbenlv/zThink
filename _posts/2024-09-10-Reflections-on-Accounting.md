...
- **double-entry bookkeeping could extend beyond organizational boundaries**. Traditionally, each organization manages its own centralized ledger, meaning they can only record their income or expenses internally, classifying them as assets or liabilities. In scenarios like business transactions or government taxation, it's often necessary to audit the other party’s books through sampling to verify the accuracy of the transactions. This process is time-consuming and costly. However, with the development of blockchain technology, decentralized ledgers can make transactions between organizations more transparent. For instance, an expense recorded by one organization can automatically be reflected as income for another. These linked ledgers allow for cross-organizational, fully verifiable transactions, reducing audit and collaboration costs while significantly improving trust. This approach could be applied in areas such as government taxation and profit sharing with investors or partners.

- blockchain enables **multi-asset and multi-currency bookkeeping to become the norm**. As technology evolves, more organizations are able to create and manage new asset types, such as fungible tokens (e.g., ERC-20) and non-fungible tokens (e.g., NFTs). Traditionally, tracking these assets would incur high costs, often requiring simplification by converting them into a base currency for accounting purposes. In future blockchain-based systems, ledgers can naturally support multi-asset and multi-currency formats. Currency conversions and value calculations across assets can be done seamlessly at any point in time. This multi-currency, multi-asset approach to bookkeeping will simplify financial operations while maintaining efficiency and transparency.
